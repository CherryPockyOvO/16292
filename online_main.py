...
def start_online():
    T1 = Thread(target=sensevoice_main).start()
    T2 = Thread(target=run_state_web).start()
    T3 = Thread(target=run_live2d).start()
    T4 = Thread(target=text_chat).start()
    os.system('./login.sh')
    while True:
        time.sleep(1)
    T1.join()
